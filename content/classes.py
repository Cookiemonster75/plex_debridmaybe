...
class media:
    ...
    def downloaded(self):
        ...
        if self.type == "movie" or self.type == "episode":
            ...
            filemode = False
            # seasons are marked as downloaded through the parent show's multi-season pack -
            # in that case the season itself has no Releases attribute, so fall back to marking
            # all episodes as downloaded (the pack contains them).
            release_files = self.Releases[0].files if hasattr(self, 'Releases') and len(self.Releases) > 0 else []
            for episode in self.Episodes:
                episode.version = self.version
